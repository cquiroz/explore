  ...
  return {
    ...
    xyToView: function(x, y, width, height, largestDim, zoomFactor, round) {
      ...
      if (round) {
        ...
        return {
          vx: AladinUtils.myRound(
            (largestDim / 2) * (1 + zoomFactor * x) - (largestDim - width) / 2
          ),
          vy: AladinUtils.myRound(
            (largestDim / 2) * (1 + zoomFactor * y) - (largestDim - height) / 2
          )
        };
      } else {
        return {
    ...
    isHpxPixVisible: function(pixCorners, viewWidth, viewHeight) {
      for (var i = 0; i < pixCorners.length; i++) {
        if (
          pixCorners[i].vx >= -20 &&
          pixCorners[i].vx < viewWidth + 20 &&
          pixCorners[i].vy >= -20 &&
          pixCorners[i].vy < viewHeight + 20
        ) {
          return true;
        }
      }
