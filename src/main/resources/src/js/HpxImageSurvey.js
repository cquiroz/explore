 ...
 *****************************************************************************/
var $ = require("jquery");
var HiPSDefinition = require("./HiPSDefinition");
var ColorMap = require("./ColorMap");
var Utils = require("./Utils");
var CooFrameEnum = require("./CooFrameEnum");
var HpxKey = require("./HpxKey");
...
HpxImageSurvey = (function() {
  ...
  var HpxImageSurvey = function(
    ...
    if (idOrHiPSDefinition instanceof HiPSDefinition) {
      ...
      if (
        ...
        (/u-strasbg.fr/i.test(this.rootUrl) || /unistra.fr/i.test(this.rootUrl))
      ) {
  ...
  HpxImageSurvey.getSurveyFromId = function(id) {
    ...
    if (surveyInfo) {
      ...
      if (
        surveyInfo.format &&
        surveyInfo.format.indexOf("jpeg") < 0 &&
        surveyInfo.format.indexOf("png") >= 0
      ) {
        options.imgFormat = "png";
      }
      return new HpxImageSurvey(
        surveyInfo.id,
        surveyInfo.name,
        surveyInfo.url,
        surveyInfo.frame,
   ...
   *
   */
  HpxImageSurvey.prototype.draw = function(
    ctx,
    view,
    subdivide,
    curOverlayNorder
    ...
    if (curOverlayNorder >= 3) {
      if (curOverlayNorder == 3) {
        ...
        cornersXYViewMapHighres = view.getVisibleCells(
          norder4Display,
          this.cooFrame
        );
      }
...
*/
  ...
  HpxImageSurvey.prototype.drawHighres = function(
    ctx,
    cornersXYViewMap,
    norder,
    view
    ...
    for (var k = 0; k < cornersXYViewMap.length; k++) {
      ...
      if (!tileAvailable) {
        ...
        for (
          var parentOrder = norder - 1;
          parentOrder >= 3 && parentOrder >= norder - MAX_UPPER_LEVELS;
          parentOrder--
        ) {
    ...
    // draw tiles
    for (var k = 0; k < cornersXYViewMap.length; k++) {
      new HpxKey(norder, cornersXYViewMap[k].ipix, this, tSize, tSize).draw(
        ctx,
        view
      );
    }
  ...
  HpxImageSurvey.prototype.drawAllsky = function(
    ctx,
    cornersXYViewMap,
    norder,
    view
  ...
  HpxImageSurvey.prototype.redrawAllsky = function(
    ctx,
    cornersXYViewMap,
    fov,
    norder
  ...
  HpxImageSurvey.prototype.redrawHighres = function(
    ctx,
    cornersXYViewMap,
    norder
  ) {
    ...
    var updateNeededTiles =
      now - this.lastUpdateDateNeededTiles >
      HpxImageSurvey.UPDATE_NEEDED_TILES_DELAY;
    ...
    if (updateNeededTiles) {
      var center = [
        (cornersXYViewMap[0][0].vx + cornersXYViewMap[0][1].vx) / 2,
        (cornersXYViewMap[0][0].vy + cornersXYViewMap[0][1].vy) / 2
      ];
      var newCornersXYViewMap = cornersXYViewMap.sort(function(a, b) {
        ...
        var distA =
          (cA[0] - center[0]) * (cA[0] - center[0]) +
          (cA[1] - center[1]) * (cA[1] - center[1]);
        var distB =
          (cB[0] - center[0]) * (cB[0] - center[0]) +
          (cB[1] - center[1]) * (cB[1] - center[1]);
      ...
      if (!tile) {
        ...
        if (parentNorder >= 3 && !parentTilesToDrawIpix[parentIpix]) {
          ...
          if (parentTile && Tile.isImageOk(parentTile.img)) {
            parentCornersXYView = this.view.getPositionsInView(
              parentIpix,
              parentNorder
            );
    ...
    for (var k = 0, len = parentTilesToDraw.length; k < len; k++) {
      this.drawOneTile(
        ctx,
        parentTilesToDraw[k].img,
        parentTilesToDraw[k].corners,
        parentTilesToDraw[k].img.width
      );
    }
    ...
    for (var k = 0, len = tilesToDraw.length; k < len; k++) {
      ...
      if (img.fadingStart) {
        if (img.fadingEnd && now < img.fadingEnd) {
          alpha =
            0.2 +
            ((now - img.fadingStart) / (img.fadingEnd - img.fadingStart)) * 0.8;
          this.requestRedraw();
        }
      }
    ...
    if (updateNeededTiles) {
      ...
      for (var k = 0, len = tilesToDownload.length; k < len; k++) {
        this.view.downloader.requestDownload(
          tilesToDownload[k].img,
          tilesToDownload[k].url,
          this.useCors
        );
      }
      ...
      for (var k = 0, len = parentTilesToDownload.length; k < len; k++) {
        this.view.downloader.requestDownload(
          parentTilesToDownload[k].img,
          parentTilesToDownload[k].url,
          this.useCors
        );
      }
    ...
    if (missingTiles) {
      ...
      this.view.requestRedrawAtDate(
        now + HpxImageSurvey.UPDATE_NEEDED_TILES_DELAY + 10
      );
    }
  };
  ...
  HpxImageSurvey.prototype.drawOneTile = function(
    ctx,
    img,
    cornersXYView,
    textureSize,
  ...
  HpxImageSurvey.prototype.drawOneTile2 = function(
    ctx,
    img,
    cornersXYView,
    textureSize,
  ...
  function drawTexturedTriangle2(
    ctx,
    img,
    x0,
    y0,
