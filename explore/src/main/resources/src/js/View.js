...
var HealpixGrid = require("./HealpixGrid");
var HealpixIndex = require("./healpix.min");
var CooFrameEnum = require("./CooFrameEnum");
var CooConversion = require("./CooConversion");
var AladinUtils = require("./AladinUtils");
var Coo = require("./Coo");
...
View = (function() {
  ...
  function View(aladin, location, fovDiv, cooFrame, zoom) {
    ...
    $(self.aladinDiv).resize(function() {
      println("Resized");
      clearTimeout(self.resizeTimer);
      self.resizeTimer = setTimeout(function() {
        self.fixLayoutDimensions(self);
  ...
  doComputeFov = function(view, zoomFactor) {
    ...
    if (view.zoomFactor < 1) {
      ...
      var xy2 = AladinUtils.viewToXy(
        view.imageCanvas.width - 1,
        view.cy,
        view.width,
        view.height,
        view.largestDim,
      ...
      fov = new Coo(lonlat1.ra, lonlat1.dec).distance(
        new Coo(lonlat2.ra, lonlat2.dec)
      );
    }
  ...
  createListeners = function(view) {
    ...
    $(view.reticleCanvas).bind("mousedown touchstart", function(e) {
      ...
      if (
        ...
        e.originalEvent.targetTouches.length == 2
      ) {
        view.dragging = false;
    ...
    $(view.reticleCanvas).bind("click mouseout touchend", function(e) {
      ...
      if (view.dragging) {
        ...
        if (wasDragging) {
          ...
          var posChangedFn =
            view.aladin.callbacksByEventName["positionChanged"];
      ...
      if (view.mode == View.TOOL_SIMBAD_POINTER) {
        ...
        SimbadPointer.query(
          radec[0],
          radec[1],
          Math.min(1, (15 * view.fov) / view.largestDim),
          view.aladin
        );
      ...
      if (!wasDragging && objs) {
        ...
        var objClickedFunction =
          view.aladin.callbacksByEventName["objectClicked"];
    ...
    $(view.reticleCanvas).bind("mousemove touchmove", function(e) {
      ...
      if (
        ...
        view.setZoom(
          (view.pinchZoomParameters.initialFov *
            view.pinchZoomParameters.initialDistance) /
            dist
        );
      ...
      if (!view.dragging || hasTouchEvents) {
        ...
        if (!view.dragging && !view.mode == View.SELECT) {
          ...
          if (closest) {
            ...
            var objHoveredFunction =
              view.aladin.callbacksByEventName["objectHovered"];
            if (
              typeof objHoveredFunction === "function" &&
              closest[0] != lastHoveredObject
            ) {
    ...
    view.executeCallbacksThrottled = Utils.throttle(function() {
      ...
      if (ra !== this.ra || dec !== this.dec) {
        ...
        typeof posChangedFn === "function" &&
          posChangedFn({ ra: ra, dec: dec, dragging: true });
  ...
  View.prototype.getViewParams = function() {
    var resolution =
      this.width > this.height ? this.fov / this.width : this.fov / this.height;
    return {
  ...
  View.prototype.redraw = function() {
    ...
    if (this.displayHpxGrid) {
      ...
      if (cornersXYViewMapHighres && this.curNorder > 3) {
        this.healpixGrid.redraw(
          imageCtx,
          cornersXYViewMapHighres,
          this.fov,
          this.curNorder
        );
      } else {
    ...
    if (
      ...
      this.displayCatalog &&
      (!this.dragging || View.DRAW_SOURCES_WHILE_DRAGGING)
    ) {
    ...
    if (
      this.mocs &&
      this.mocs.length > 0 &&
      (!this.dragging || View.DRAW_MOCS_WHILE_DRAGGING)
    ) {
      if (!catalogCanvasCleared) {
        catalogCtx.clearRect(0, 0, this.width, this.height);
        catalogCanvasCleared = true;
    ...
    if (
      this.projectionMethod == ProjectionEnum.SIN &&
      this.fov >= 60 &&
      this.aladin.options["showAllskyRing"] === true
    ) {
      ...
      imageCtx.arc(
        this.cx,
        this.cy,
        (maxCxCy - ringWidth / 2.0 + 1) * this.zoomFactor,
        0,
        2 * Math.PI,
        true
      );
  ...
  View.prototype.getVisibleCells = function(norder, frameSurvey) {
    ...
    for (var ipixIdx = 0, len = pixList.length; ipixIdx < len; ipixIdx++) {
      ...
      for (var k = 0; k < 4; k++) {
        ...
        if (frameSurvey && frameSurvey.system != this.cooFrame.system) {
          if (frameSurvey.system == CooFrameEnum.SYSTEMS.J2000) {
            var radec = CooConversion.J2000ToGalactic([
              spVec.ra(),
              spVec.dec()
            ]);
            lon = radec[0];
      ...
      for (var k = 0; k < 4; k++) {
        cornersXYView[k] = AladinUtils.xyToView(
          cornersXY[k].X,
          cornersXY[k].Y,
          this.width,
          this.height,
      ...
      if (
        cornersXYView[0].vx < 0 &&
        cornersXYView[1].vx < 0 &&
        cornersXYView[2].vx < 0 &&
        cornersXYView[3].vx < 0
      ) {
        continue;
      }
      if (
        cornersXYView[0].vy < 0 &&
        cornersXYView[1].vy < 0 &&
        cornersXYView[2].vy < 0 &&
        cornersXYView[3].vy < 0
      ) {
        continue;
      }
      if (
  ...
  View.prototype.computeZoomFactor = function(level) {
    if (level > 0) {
      return AladinUtils.getZoomFactorForAngle(
        180 / Math.pow(1.15, level),
  ...
  View.prototype.setZoomLevel = function(level) {
    if (this.minFOV || this.maxFOV) {
      var newFov = doComputeFov(
        this,
        this.computeZoomFactor(Math.max(-2, level))
      );
      if (
        (this.maxFOV && newFov > this.maxFOV) ||
        (this.minFOV && newFov < this.minFOV)
      ) {
        return;
      }
    }
  ...
  View.prototype.setOverlayImageSurvey = function(
    overlayImageSurvey,
    callback
  ) {
    ...
    if (typeof overlayImageSurvey == "string") {
      newOverlayImageSurvey = HpxImageSurvey.getSurveyFromId(
        overlayImageSurvey
      );
  ...
  View.prototype.setImageSurvey = function(imageSurvey, callback) {
    ...
    if (typeof imageSurvey == "string") {
      ...
      if (!newImageSurvey) {
        newImageSurvey = HpxImageSurvey.getSurveyFromId(
          HpxImageSurvey.DEFAULT_SURVEY_ID
        );
    ...
    if (
      newImageSurvey &&
      this.imageSurvey &&
      newImageSurvey.hasOwnProperty("id") &&
      this.imageSurvey.hasOwnProperty("id") &&
      newImageSurvey.id == this.imageSurvey.id
    ) {
      // do nothing
    } else {
      // buffer reset
  ...
  View.prototype.changeFrame = function(cooFrame) {
    ...
    if (
      ...
      this.cooFrame.system != oldCooFrame.system
    ) {
      var lb = CooConversion.J2000ToGalactic([
        this.viewCenter.lon,
        this.viewCenter.lat
      ]);
      ...
      for (var k = 0; k < this.overlays.length; k++) {
        ...
        for (var i = 0; i < overlay.overlays.length; i++) {
          ...
          for (var j = 0; j < footprint.polygons.length; j++) {
            var xy = AladinUtils.radecToViewXy(
              footprint.polygons[j][0],
              footprint.polygons[j][1],
              this.projection,
              this.cooFrame,
              this.width,
              this.height,
