...
HiPSDefinition = (function() {
  ...
  var HiPSDefinition = function(properties) {
    ...
    this.tileFormats =
      properties.hasOwnProperty("hips_tile_format") &&
      properties["hips_tile_format"].split(" ");
    this.urls = [];
  ...
  HiPSDefinition.storeInLocalStorage = function(properties) {
    try {
      window.localStorage.setItem(
        HiPSDefinition.LOCAL_STORAGE_KEY,
        window.JSON.stringify(properties)
      );
    } catch (e) {
      console.error(e);
      return false;
  ...
  HiPSDefinition.getRemoteDefinitions = function(
    params,
    successCallbackFn,
    failureCallbackFn
  ) {
    ...
    var urls = Utils.isHttpsContext()
      ? MOCSERVER_MIRRORS_HTTPS
      : MOCSERVER_MIRRORS_HTTP;
  ...
  var merge = function(baseList, newList) {
    ...
    for (var k = 0; k < baseList.length; k++) {
      ...
      if (newListById.hasOwnProperty(id)) {
        ...
        if (
          item.hasOwnProperty(LAST_URL_KEY) &&
          !itemToAdd.hasOwnProperty(LAST_URL_KEY)
        ) {
  ...
  HiPSDefinition.init = function() {
    ...
    for (var k = 0; k < localDefs.length; k++) {
      ...
      if (
        ...
        now - def[RETRIEVAL_TIMESTAMP_KEY] >
          1000 * HiPSDefinition.CACHE_RETENTION_TIME_SECONDS
      ) {
  ...
  HiPSDefinition.getALDefaultHiPSDefinitions = function() {
    ...
    for (var k = 0; k < listHipsProperties.length; k++) {
      ...
      if (
        !properties.hasOwnProperty("client_application") ||
        properties["client_application"].indexOf("AladinLite") < 0
      ) {
        continue;
      }
